...
def get_batch(split):
    ...
    mask = np.memmap(os.path.join(data_dir, f'{split}_mask.bin'),
    ...
    for i in ix:
        ...
        y.append(torch.from_numpy((data[i+1:i+1+block_size]).astype(np.int64)))
        m.append(torch.from_numpy((mask[i:i+block_size]).astype(np.int64)))
